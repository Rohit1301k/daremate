...
const Components = {
    ...
    gameRoom() {
        ...
        // Voice note button
        const voiceBtn = UI.createElement('button', [
            'text-gray-400', 'hover:text-white', 'p-2', 'rounded-full',
            'hover:bg-gray-700', 'transition-colors'
        ], {
            type: 'button',
        ...
        voiceBtn.addEventListener('click', async () => {
            try {
                if (!isRecording) {
                    ...
                    mediaRecorder.onstop = async () => {
                        ...
                        reader.onload = async (e) => {
                            ...
                            await Game.sendMessage(`[voice:${base64}]`);
                        };
                ...
                UI.showNotification('Could not access microphone. Please check permissions.', 'error');
            }
        });
    ...
    createChatMessage(message) {
        const messageEl = UI.createElement('div', [
            ...
            message.system ? 'message-system' :
                (message.player_id === this.player?.id ? 'message-self' : 'message-others')
        ]);
        ...
        // Get player info
        const player = Database.getById('players', message.player_id);
        
        // Add player name if it's not the current player
        if (player && player.id !== this.player?.id) {
            const nameEl = UI.createElement('div', ['text-xs', 'text-gray-400', 'mb-1']);
            nameEl.textContent = player.nickname;
            messageEl.appendChild(nameEl);
        }

        // Create container for media content and pin icon
        const contentContainer = UI.createElement('div', ['relative']);
        
        // Check message type and create appropriate content
        if (message.message.startsWith('[voice:')) {
            const base64Audio = message.message.slice(7, -1);
            const audioContainer = UI.createElement('div', ['flex', 'items-center', 'space-x-2']);
            
            // Audio player
            const audio = UI.createElement('audio', ['max-w-[200px]'], {
                controls: true,
                src: base64Audio,
                preload: 'metadata'
            });
            ...
            // Add play icon
            const icon = UI.createElement('i', ['fas', 'fa-play-circle', 'text-purple-500', 'text-xl']);
            
            audioContainer.appendChild(icon);
            audioContainer.appendChild(audio);
            contentContainer.appendChild(audioContainer);

            // Add pin icon for voice message
            const pinIcon = UI.createElement('button', [
            ...
            pinIcon.addEventListener('click', () => {
                UI.showNotification('Voice note pinned!');
            });
            contentContainer.appendChild(pinIcon);
        }
        else if (message.message.startsWith('[image:')) {
            const base64Image = message.message.slice(7, -1);
            const img = UI.createElement('img', ['max-w-full', 'rounded'], {
                src: base64Image,
                alt: 'Shared image'
            });
            contentContainer.appendChild(img);

            // Add pin icon for image
            const pinIcon = UI.createElement('button', [
                'absolute', 'top-2', 'right-2', 'text-gray-400',
                'hover:text-white', 'transition-colors', 'z-10',
            ...
            pinIcon.innerHTML = '<i class="fas fa-thumbtack"></i>';
            pinIcon.addEventListener('click', () => {
                UI.showNotification('Image pinned!');
            });
            contentContainer.appendChild(pinIcon);
        }
        else if (message.message.startsWith('[gif:')) {
            const gifUrl = message.message.slice(5, -1);
            const img = UI.createElement('img', ['max-w-full', 'rounded'], {
                src: gifUrl,
                alt: 'GIF'
            });
            ...
            pinIcon.innerHTML = '<i class="fas fa-thumbtack"></i>';
            pinIcon.addEventListener('click', () => {
                UI.showNotification('GIF pinned!');
            });
            contentContainer.appendChild(pinIcon);
        }
        else {
